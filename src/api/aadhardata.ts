...
interface FetchAndProcessOptionsArgs {
    apiUrl: string;
    labelPath: string;
    valuePath: string;
}

export async function fetchAndProcessOptions({ apiUrl, labelPath, valuePath }: FetchAndProcessOptionsArgs) {
    try {
        ...
        console.error('Failed to fetch and process options', error);
        throw error;
    }
}
