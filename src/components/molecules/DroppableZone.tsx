...
const DroppableZone: React.FC = () => {
  ...
  return (
    <div
      ref={setNodeRef}
      className={cn(
        "flex flex-col min-h-full border-dashed border-2 border-gray-300 p-2 py-4 bg-gray-100",
        {
    ...
    >
      {isOver && !data?.length && (
        <div className="bg-gray-300 w-full p-5 rounded" />
      )}

      {data?.length == 0 && (
        <div className="w-full font-semibold text-gray-500 text-center flex-grow h-full flex items-center">
          <h1 className="w-full font-semibold text-gray-500 text-center flex-grow">
            Drag items here
          </h1>
        </div>
      )}
